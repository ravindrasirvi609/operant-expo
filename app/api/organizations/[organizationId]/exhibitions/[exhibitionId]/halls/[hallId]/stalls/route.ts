...
export async function POST(request: Request, { params }: { params: Promise<{ organizationId: string; exhibitionId: string; hallId: string }> }) {
  ...
  try { await database.collection<StallDocument>("stalls").insertOne(stall); return NextResponse.json({ stall }, { status: 201 }); } catch (error) { if (error instanceof MongoServerError && error.code === 11000) return NextResponse.json({ error: "Stall number already exists in this hall" }, { status: 409 }); return NextResponse.json({ error: "Unable to create stall" }, { status: 500 }); }
}
