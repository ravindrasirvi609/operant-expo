...
/**
 * A visitor's temporary claim on a stall.
 *
 * `visitorId` is what makes a reservation resumable: without an owner, the hold endpoint could not
 * tell the holder apart from anyone else, so it demanded an AVAILABLE stall and refused the very
 * visitor who was already holding it.
 */
export type ReservationHoldDocument = {
  _id?: ObjectId;
